...
// SELECT PERSONA.*, USUARIO.*, REGISTRO_ROL.*, ROL.NOMBRE_ROL FROM PERSONA JOIN USUARIO ON PERSONA.ID_PERSONA = USUARIO.ID_USUARIO JOIN REGISTRO_ROL ON USUARIO.FK_ID_REGISTRO_ROL = REGISTRO_ROL.ID_REGISTRO_ROL JOIN ROL ON REGISTRO_ROL.FK_ID_ROL = ROL.ID_ROL WHERE PERSONA.IDENTIFICACION = ?

const getAllOrders = () => {
    return new Promise((resolve, reject) => {
        pool.query(`SELECT PEDIDO.*, DETALLE_PEDIDO.*, PRODUCTO.*, PERSONA.*, TIPO_ENTREGA.DESCRIPCION_ENTREGA, DOMICILIO.COSTO_DOMICILIO, DOMICILIO.DESCRIPCION_DOMICILIO, ESTADO_PEDIDO.DESCRIPCION_ESTADO_PEDIDO, ESTADO_PAGO_PEDIDO.DESCRIPCION_ESTADO, ADICION_DETALLE_PEDIDO.CANTIDAD_ADICION, ADICION.NOMBRE_ADICION, SABOR_DETALLE_PEDIDO.*, SABOR.NOMBRE_SABOR FROM PEDIDO JOIN PERSONA ON PEDIDO.FK_ID_PERSONA = PERSONA.ID_PERSONA JOIN ESTADO_PEDIDO ON PEDIDO.FK_ID_ESTADO_PEDIDO = ESTADO_PEDIDO.ID_ESTADO_PEDIDO JOIN TIPO_ENTREGA ON PEDIDO.FK_ID_TIPO_ENTREGA = TIPO_ENTREGA.ID_TIPO_ENTREGA LEFT JOIN DOMICILIO ON PEDIDO.FK_ID_DOMICILIO = DOMICILIO.ID_DOMICILIO JOIN ESTADO_PAGO_PEDIDO ON PEDIDO.FK_ID_ESTADO_PAGO = ESTADO_PAGO_PEDIDO.ID_ESTADO_PAGO JOIN DETALLE_PEDIDO ON DETALLE_PEDIDO.FK_ID_PEDIDO = PEDIDO.ID_PEDIDO JOIN PRODUCTO ON DETALLE_PEDIDO.FK_ID_PRODUCTO = PRODUCTO.ID_PRODUCTO LEFT JOIN ADICION_DETALLE_PEDIDO ON ADICION_DETALLE_PEDIDO.FK_ID_DETALLE_PEDIDO = DETALLE_PEDIDO.ID_DETALLE_PEDIDO LEFT JOIN ADICION ON ADICION_DETALLE_PEDIDO.FK_ID_ADICION = ADICION.ID_ADICION LEFT JOIN SABOR_DETALLE_PEDIDO ON SABOR_DETALLE_PEDIDO.FK_ID_DETALLE_PEDIDO = DETALLE_PEDIDO.ID_DETALLE_PEDIDO LEFT JOIN SABOR ON SABOR_DETALLE_PEDIDO.FK_ID_SABOR = SABOR.ID_SABOR ORDER BY PEDIDO.ID_PEDIDO, DETALLE_PEDIDO.ID_DETALLE_PEDIDO`, (error, result) => {
            if (error) {
                reject(error);
            } else {
